...
engine = create_engine('postgresql://catalog:rahulkathuriad@localhost/catalog')
...
def fbconnect():
    ...
    '''
        ...
        token value and replace the remaining quotes with nothing\
...
def showRestaurants():
    ...
    if 'username' not in login_session:
        return render_template('publicrestauran'
                               'ts.html', restaurants=restaurants)
    else:
...
def editRestaurant(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editRestaurant.'
                               'html', restaurant=editedRestaurant)
...
def showMenu(restaurant_id):
    ...
    items = session.query(MenuItem).filter_by(
        restaurant_id=restaurant_id).all()
    if 'username' not in login_session or creator.id != login_session['use'
                                                                      'r_id']:
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        ...
        add menu items to this restaurant. Please create your own \
        restaurant in order to add items.');}</script>\
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        ...
        own restaurant in order to edit items.');}\
        </script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete menu \
        items to this restaurant. Please create your own\
